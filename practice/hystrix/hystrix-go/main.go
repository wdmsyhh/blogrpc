...
func CircuitBreakerWrapper(ctx *gin.Context) {
	...
	fmt.Println("CircuitBreakerName:", name) // api_/api/ping/baidu_circuit_breaker
	hystrix.Do(name, func() error {
		...
		if err != nil {
			// 监控上报（未实现）
			_, err2 := io.WriteString(f, fmt.Sprintf("circuitBreaker and err is %s\n", err.Error())) //写入文件(字符串)
			if err2 != nil {
...
func init() {
	name := fmt.Sprintf(CircuitBreakerName, "/api/ping/baidu")
	...
	if checkFileIsExist(filename) { //如果文件存在
		f, errfile = os.OpenFile(filename, os.O_APPEND|os.O_WRONLY, 0666) //打开文件
	} else {
...
func main() {
	...
	r.Run(":8888") // 不填地址默认： listen and serve on 0.0.0.0:8080 (for windows "localhost:8080")
}
